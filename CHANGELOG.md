...
- **Scheduled alert emails** — the alert-rules-evaluator cron needs its own `RESEND_API_KEY` and `TELEMETRY_EMAIL_FROM` (same values as the API). Missing config no longer fails silently: sweep logs include `email: "unavailable"`, and notification email failures are logged without secret values
